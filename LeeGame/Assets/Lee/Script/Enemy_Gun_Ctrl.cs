...
public class Enemy_Gun_Ctrl : MonoBehaviour
{
    ...
    private float fAttackDist = 5.0f;
    // 적군과 아군 사이의 일정 거리가 되면 적이 아군을 추적하기 시작함
    private float fTraceDist = 7.0f;
    ...
    void Update()
    {
        ...
        else if (state == ENEMYSTATE.ENEMYATTACK)
        {
            ...
            if (false == animator.GetBool("Attack") && GameObject.FindWithTag("Player").GetComponent<Player_Ctrl>().PS == PlayerState.Run)
            {
                BeginAttack();
                if (animator.GetBool("Attack") == true && animator.GetBool("Die") == false)
                      StartCoroutine(this.bulletAttack());
            }
            else
            {
    ...
    IEnumerator bulletAttack()
    {
        ...
        vPosition.Set(vPosition.x - 1.5f, fCurrentYPosotion, vPosition.z);
        Instantiate(bullet, vPosition, this.trBulletPos.rotation);
        yield return null;
    }
    ...
    void OnEnable()
    {
        Player_Ctrl.onPlayerDie += this.onPlayerDie;
    }
